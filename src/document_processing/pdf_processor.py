...
# CONFIG
# -----------------------------------------
PDF_DIR = "data/documents/pdfs"   # change if needed
LOG_LEVEL = "INFO"
...
def upsert_pdf(conn, *, file_name, file_path, file_hash, file_size,
               total_pages, metadata_json, embedding_model, vector_table):
    sql = text("""
...
def mark_completed(conn, pdf_id: int, total_chunks: int):
    conn.execute(text("""
        ...
        SET processing_status='completed',
            metadata = metadata || jsonb_build_object('vector_rows', :rows)
        WHERE id = :id
...
def process_pdf(engine, embedder, pdf_path: Path):
    ...
    # Upsert pdf record and recreate table
        ...
        for i in range(0, len(chunks), BATCH):
            ...
            for (page_no, text_chunk), emb in zip(batch, embs):
                ...
                insert_chunk(
                    ...
                    tokens=tokens,
                    page_no=page_no,
                    metadata_obj={"file": pdf_path.name},
    ...
    logger.info(f"Completed {pdf_path.name}: {inserted} chunks inserted.")
